...
AI is optional. Set `AI_PROVIDER=gemini`, `GEMINI_API_KEY`, and `GEMINI_MODEL` to enable structured note interpretation. Add `OPENROUTER_API_KEY` (or `OPEN_ROUTER_API_KEY`) to automatically use OpenRouter's free router when Gemini is unavailable. `OPENROUTER_MODEL` defaults to `openrouter/free`. Notes save immediately even when AI is disabled or unavailable.
